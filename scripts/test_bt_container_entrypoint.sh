...
grep -Fq ': "${BT_REQUIRED_ARMS:=$REALMAN_BT_ARM_ID}"' "$ENTRYPOINT"
grep -Fq ': "${BT_LAUNCH_FILE:=arm_move.launch.py}"' "$ENTRYPOINT"
grep -Fq ': "${BT_STOP_ON_TERMINAL:=true}"' "$ENTRYPOINT"
grep -Fq ': "${BT_EXIT_ON_TERMINAL:=true}"' "$ENTRYPOINT"
...
grep -Fq 'case "$BT_LAUNCH_FILE" in' "$ENTRYPOINT"
grep -Fq 'arm_move.launch.py|control_router.launch.py)' "$ENTRYPOINT"
