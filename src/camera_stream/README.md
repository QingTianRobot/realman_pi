...
（深度分辨率/端口/lz4）。Orbbec 支持多台：`orbbec.yaml` 里 `cameras.left/right/...` 按 side 名
...
python3 -m camera_stream.orbbec_stream   config/orbbec.yaml --side left   # 或 --side right
...
- **USB 带宽**：手眼 Gemini 305 在 USB 2.0（480M）上原始 BGR 彩色+深度会超带宽导致彩色饿死，
  所以默认用 `format: MJPG` 压缩彩色（实测 USB 2.0 下彩色+深度都能 30fps）。换 USB 3.0 口后可把
...
- **带宽**：两台彩色(H.264)+深度(uint16)满跑约数百 Mbps，走千兆网卡（192.168.5.80）无压力；
