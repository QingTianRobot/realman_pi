...
grep -Fq ': "${BT_REQUIRED_ACTIONS:=}"' "$ENTRYPOINT"
...
grep -Fq 'IFS="," read -r -a required_actions <<<"$BT_REQUIRED_ACTIONS"' "$ENTRYPOINT"
grep -Fq 'action_name="/${arm_id}/${action}"' "$ENTRYPOINT"
