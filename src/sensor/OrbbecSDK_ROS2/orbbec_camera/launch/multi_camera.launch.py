from launch import LaunchDescription
from launch.actions import DeclareLaunchArgument, IncludeLaunchDescription, GroupAction, ExecuteProcess, TimerAction
from launch.launch_description_sources import PythonLaunchDescriptionSource
from launch_ros.actions import Node
from ament_index_python.packages import get_package_share_directory
import os


def generate_launch_description():
    # Include launch files
    package_dir = get_package_share_directory('orbbec_camera')
    launch_file_dir = os.path.join(package_dir, 'launch')
    launch1_include = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(launch_file_dir, 'gemini_330_series.launch.py')
        ),
        launch_arguments={
            'camera_name': 'camera_01_CP2AB530006D',
            'usb_port': '1-2',
            'device_num': '3',
            'sync_mode': 'standalone',
            'enable_left_ir': 'true',
            'enable_right_ir': 'true',
            'log_level': 'none',
            'log_file_name': 'camera_01.log',
        }.items()
    )

    launch2_include = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(launch_file_dir, 'gemini_330_series.launch.py')
        ),
        launch_arguments={
            'camera_name': 'camera_02_CP2AB5300086',
            'usb_port': '1-6',
            'device_num': '3',
            'sync_mode': 'standalone',
            'enable_left_ir': 'true',
            'enable_right_ir': 'true',
            'log_level': 'none',
            'log_file_name': 'camera_02.log',
        }.items()
    )

    launch3_include = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(launch_file_dir, 'gemini_330_series.launch.py')
        ),
        launch_arguments={
            'camera_name': 'camera_03_CP2N163000HK',
            'usb_port': '1-1',
            'device_num': '3',
            'sync_mode': 'standalone',
            'enable_left_ir': 'true',
            'enable_right_ir': 'true',
            'log_level': 'none',
            'log_file_name': 'camera_03.log',
        }.items()
    )

    # If you need more cameras, just add more launch_include here, and change the usb_port and device_num

    # Launch description
    ld = LaunchDescription([
            TimerAction(period=0.0, actions=[GroupAction([launch2_include])]),
            TimerAction(period=0.0, actions=[GroupAction([launch3_include])]),
            TimerAction(period=0.0, actions=[GroupAction([launch1_include])]),
    ])

    return ld
