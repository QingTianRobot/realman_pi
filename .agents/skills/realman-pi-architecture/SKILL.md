...
description: Use when working on the realman_pi repository's startup flow, ROS 2 graph, camera/robot/Web control integration, production deployment, or architecture questions.
---
...
加载本 skill 后，先把它当作项目运行契约，再阅读具体源文件。涉及启动、部署、相机、机械臂、Web control、标定或 ROS 图时必须遵守其中的边界；涉及 RealMan Python SDK 细节时另用 `realman-python-driver`。
...
Docker Compose / ROS 2 Humble
  ...
  └─ realman_web_control（HTTP/WebSocket :8765）
       └─ 订阅 /camera_calibration/camera_health、joint_states、TF/坐标状态
```
